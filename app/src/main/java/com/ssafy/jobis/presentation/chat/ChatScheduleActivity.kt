...
class ChatScheduleActivity : AppCompatActivity() {
    ...
    private fun updateStudyScheduleList(scheduleList: MutableList<ScheduleResponse>) {
        ...
        for (i: Int in 0..scheduleList.size-1) {
            ...
            println("scheduleList[${i}]" + scheduleList[i].studyId)
            if (scheduleList[i].studyId != 0L) { // studyId가 있는 경우만
                ...
                var groupId = scheduleList[i].groupId.toInt()
                var studyId = scheduleList[i].studyId.toInt()
